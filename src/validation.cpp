...
public:
    ...
    bool ActivateBestChain(CValidationState& state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock, int* fork=nullptr);
...
private:
    bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace, int* fork=nullptr);
 ...
 */
bool CChainState::ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace, int* fork)
{
    AssertLockHeld(cs_main);

    const CBlockIndex *pindexOldTip = chainActive.Tip();
    const CBlockIndex *pindexFork = chainActive.FindFork(pindexMostWork);
	if(pindexFork != nullptr && fork != nullptr && pindexFork->nHeight + 1 < *fork){
		*fork = pindexFork->nHeight + 1;
	}
 ...
 */
bool CChainState::ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock, int* fork) {
    ...
    do {
...

        {
            ...
            do {
                ...
                if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace, fork))
                    return false;
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
	...
	static int kafkaHeightRange = chainActive.Height() + 1;//kafkaHeightRange must be initialized before ActivateBestChain()
	
    CValidationState state; // Only used to report errors, not invalidity - ignore it
    if (!g_chainstate.ActivateBestChain(state, chainparams, pblock, &kafkaHeightRange))
        return error("%s: ActivateBestChain failed (%s)", __func__, FormatStateMessage(state));

    //send block to kafka order by height
    if (gArgs.IsArgSet("-kafka")) {
        myPrintBlockOrderByHeight(kafkaHeightRange);
    }
...
void myPrintBlockOrderByHeight(int& kafkaHeightRange)
{
	if(kafkaHeightRange <= chainActive.Height())
	{
		for(int i = kafkaHeightRange; i <= chainActive.Height(); i++)
		{
            ...
            int ret = post(gArgs.GetArg("-kafkaproxyhost", "localhost"), gArgs.GetArg("-kafkaproxyport", "8082"), "/topics/" + gArgs.GetArg("-kafkatopic", "test"), "{\"records\":[{\"value\":" + myGetBlock(i).write() + "}]}", response_data);
            if (ret != 0)
            {
